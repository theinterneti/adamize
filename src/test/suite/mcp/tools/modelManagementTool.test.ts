...
describe('ModelManagementTool', () => {
  ...
  /**
   * @test TEST-MODEL-031e-error Test that the pullModel function handles errors correctly
   */
  test('should handle errors when pulling model', async () => {
    // Setup mock to throw an error
    const mockError = new Error('Failed to pull model');
    mockModelManager.pullOllamaModel = jest.fn().mockRejectedValue(mockError);
    ...
    // Verify result
    expect(result.status).toBe('error');
    expect(result.error).toContain('Failed to pull model');
  ...
  /**
   * @test TEST-MODEL-031f-error Test that the removeModel function handles errors correctly
   */
  test('should handle errors when removing model', async () => {
    // Setup mock to throw an error
    const mockError = new Error('Failed to remove model');
    mockModelManager.removeOllamaModel = jest.fn().mockRejectedValue(mockError);

    // Execute function
    const result = await modelManagementTool.execute('removeModel', { modelName: 'model1' });

    // Verify result
    expect(result.status).toBe('error');
    expect(result.error).toContain('Failed to remove model');
